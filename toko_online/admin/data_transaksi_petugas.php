...
?>
<h2>Histori pembalian barang</h2>
<table class="table table-hover table-striped">
    ...
    <tbody>
        <?php 
        include "../koneksi.php";
        $qry_histori=mysqli_query($konn,"select * from transaksi order by id_transaksi desc");
        $no=0;
        while($dt_histori=mysqli_fetch_array($qry_histori)){
            $no++;
            //menampilkan barang yang dibeli
            $barang_beli="<ol>";
            $qry_produk=mysqli_query($konn,"select * from  detail_transaksi join produk on produk.id_produk=detail_transaksi.id_produk where id_transaksi = '".$dt_histori['id_transaksi']."'");
        ...
        ?>
        <tr>
            <td><?=$no?></td><td><?=$dt_histori['tgl_transaksi']?></td><td><?=$barang_beli?></td>
            ...
            <?php   
                                               ...
                                               <div class="alert alert-secondary" role="alert">produk sedang dikemas</div>
                                                    ...
                                                    ?>
                                                <div class="alert alert-light" role="alert">produk telah diterima</div>
                                                    <?php
                                                }else{
                                                    ?>
...
<?php
    if (isset($_POST['konfirmasi'])){
        ...
        include "../koneksi.php";
        $sql = mysqli_query($konn,"update transaksi set status = 'barang sudah di konfirm' WHERE id_transaksi ='$id_transaksi' ");
        if ($sql) { 
            echo "<script>location.href='data_transaksi_petugas.php'</script>";
        }
    }
...
<?php
    if (isset($_POST['proses_dikemas'])){
        $id_transaksi = $_POST['id'];
        include "../koneksi.php";
        $sql = mysqli_query($konn,"update transaksi set status = 'produk sedang dikemas' WHERE id_transaksi ='$id_transaksi' ");
        if ($sql) { 
            echo "<script>location.href='data_transaksi_petugas.php'</script>";
        }
    }
...
<?php
    if (isset($_POST['proses_kirim'])){
        $id_transaksi = $_POST['id'];
        include "../koneksi.php";
        $sql = mysqli_query($konn,"update transaksi set status = 'produk sedang dikirim' WHERE id_transaksi ='$id_transaksi' ");
        if ($sql) { 
            echo "<script>location.href='data_transaksi_petugas.php'</script>";
        }
    }
